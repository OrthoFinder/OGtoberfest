...
def get_expected_genes(
    ...
    for fn in input_dir.iterdir():
        ...
        species = util.curate_labels(species)
...
def filter_orthogroups(ref_ogs: Dict[str, Set[str]], pred_ogs: Dict[str, Set[str]]):

    ref_ogs_set: Set[str] = set()
    for og in ref_ogs:
...
def get_num_species_dict(species_dict):
    ...
    num_species_dict = {og_key: len(og_species) for og_key, og_species in species_dict.items()}
...
def get_predog_num_species_dict(predog_species_dict):
    ...
    num_species_predog_dict = {
        refog_key: {
            predog_key: len(predog_species) for predog_key, predog_species in predogs.items()
        }
